...
namespace MidiLoader.Parser {
    ...
    public class HeaderParser {
		...
		public static HeaderInfo Parse( Stream stream ) {
			...
			headerInfo.format = ParseFormat( stream ); // 2 bytes
			headerInfo.tracksCount = ParseTracksCount( stream ); // 2 bytes
			headerInfo.deltasDivision = ParseDeltasDivision( stream ); // 2 bytes
